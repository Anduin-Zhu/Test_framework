...
class HTTPClient(object):
    ...
    def __init__(self,url,method='GET',headers=None,cookies=None):
        """headers:字典。ps:headers={'Content_Type':'text/html'},cookies也是字典"""
        self.url = url
        self.session = requests.session()
        self.method = method.upper()#upper 把小写字母转换成大写字母
...
class TCPClient(object):
    ...
    def connect(self):
        ...
        if not self.connected:
            try:
                self._sock.connect((self.domain,self.port))#给定参数必须是元组
    ...
    def send(self,data,dtype='str',suffix=''):
        """像服务器端发送send_string,并返回信息，若报错，则返回None"""
        if dtype =='json':
